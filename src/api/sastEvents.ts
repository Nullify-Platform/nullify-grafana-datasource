...
export const processSastEvents = async (
  ...
  for (let i = 0; i < MAX_API_REQUESTS; ++i) {
    ...
    const response = await request_fn('sast/events', params);
    ...
    if (!response.data.events || response.data.events.length === 0 || !response.data.nextEventId) { // No more events
      break;
    } else if(response.data.events[0].timestampUnix > range.to.unix()) { // No more events required
      break;
    } else {
  ...
  return createDataFrame({
    ...
    fields: [
      ...
      { name: 'time', type: FieldType.string, values: events.map((event) => event.time) },
      {
        name: 'timestampUnix',
        type: FieldType.time,
